...
########################################
#
# All the changes made here concern the
# ConnectFourBoardClass. Please note that
# all the original utilities have been
# left untouched with modifications only
# to accomodate a general ConnectK game
# mode and a longest streak game mode.
# The constructor accepts a new argument
# streak, which has a default value of 4,
# and determines the streak needed to
# finish a game. It has been made the
# first parameter of the constructor to
# conveniently call the constructor as
# ConnectFourBoard(k). Note, streak is
# also used to select the longets streak
# game mode when streak is set to > 7.
# This makes sense as the longest streak
# attainable is 7.
#
# Changes have had to be made to the
# functions do_move and clone, which
# call the constructor. Changes have
# only been made to the constructor calls,
# to account for the extra parameter. The
# rest are untouched.
#
# The functions is_win, is_tie have been
# modified to account for the longest streak
# game mode. They still function the same
# for a conventional ConnectK game.
#
# The function is_win_from_cell has been
# modified for a general ConnectK game.
#
# A new function get_streak has been added
# which allows external acces to the streak
# value of the ConnectFourBoard object.
#
########################################
...
class ConnectFourBoard(object):
    ...
    def __init__(self, streak = 4, board_array = None, board_already_won = None, modified_column = None, current_player = 1, previous_move = -1):
    ...
    def get_streak(self):
        """ Return the streak associated with the object"""
        ########################################
        ############# NEW ADDITION #############
        ########################################
        return self.streak
    ...
    def do_move(self, column):
        ...
        return ConnectFourBoard(self.streak, board_array=new_board, board_already_won=self.is_win(), modified_column=column, current_player = self.get_other_player_id())
    ...
    def _is_win_from_cell(self, row, col):
        ...
        ############### MODIFIED ###############
        ########################################
        return ( self._max_length_from_cell(row, col) >= self.get_streak() )
    ...
    def is_win(self):
        ...
        if self.longest_streak:
            if self.num_tokens_on_board() == 20:
                if self.longest_chain(self.get_current_player_id()) > self.longest_chain(self.get_other_player_id()):
                    self._is_win = 1
                    return self.get_current_player_id()
                elif self.longest_chain(self.get_current_player_id()) < self.longest_chain(self.get_other_player_id()):
                    self._is_win = 1
    ...
    def is_tie(self):
        ...
        ########################################
        if self.longest_streak:
            return (self.num_tokens_on_board() == 20) and (self.longest_chain(self.get_current_player_id()) == self.longest_chain(self.get_other_player_id()))
        else:
    ...
    def clone(self):
        ...
        ############### MODIFIED ###############
        ########################################
        return ConnectFourBoard(self.streak, board_array=self._board_array, board_already_won=self._is_win, current_player = self.get_current_player_id())
